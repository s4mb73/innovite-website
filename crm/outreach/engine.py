...
def _maybe_schedule_followup(email_row: dict, settings: dict, mode: str) -> None:
    ...
    # Subject for the follow-up — re: with the original subject.
    parent_subject = email_row.get("subject") or ""
    next_subject = parent_subject if parent_subject.lower().startswith("re:") else f"Re: {parent_subject}"

    # Body is operator-drafted later (or generated by the drafter at send-time);
    # for now we queue a placeholder the operator (or a later drafter pass) fills.
    # The lead's cached email_body_dayN columns will receive the real text once
    # the drafter learns the follow-up shape.
    db.execute(
        ...
        (
            ...
            next_step,
            next_subject,
            "(follow-up draft to be generated by drafter on the day)",
